...
class vec3 {
 ...
 public:
  ...
  __device__ operator vec4() const;
...
__device__ inline vec3& vec3::operator+=(const vec3& v2) {
  e[0] += v2[0];
  e[1] += v2[1];
  e[2] += v2[2];
  return *this;
...
__device__ inline vec3 operator*(const float t, const vec3& v)
{
  return vec3(v[0]*t, v[1]*t, v[2]*t);
}
...
__device__ inline bool refract(const vec3& v, const vec3& n, float n1_over_n2, vec3& refracted)
{
  vec3 uv = normalize(v);
  float dt = dot(uv, n);
...
class vec4 {
 ...
 public:
  __device__ vec4() : e{0.f, 0.f, 0.f, 1.f} {}
  __device__ vec4(float x, float y, float z, float w) : e{x, y, z, w} {}
  __device__ inline float x() { return e[0]; }
  __device__ inline float y() { return e[1]; }
  __device__ inline float z() { return e[2]; }
  __device__ inline float w() { return e[3]; }
  __device__ inline float r() { return e[0]; }
  __device__ inline float g() { return e[1]; }
  __device__ inline float b() { return e[2]; }
  __device__ inline float a() { return e[3]; }

  __device__ inline vec4 operator+() const { return *this; }
  __device__ inline vec4 operator-() const { return vec4(-e[0], -e[1], -e[2], -e[3]); }
